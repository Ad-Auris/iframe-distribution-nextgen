...
const IFRAME_URL_IDENTIFIER = "ad-auris-iframe-url";
...
function myFunction() {
    console.log("ad auris")
    // var parent_url = window.location.href;
    var parent_url = "https://racinecountyeye.com/wisconsin-governor-signs-bill-opioid-funds" 
    ...
    normalizeUrl(parent_url).then((response) => {
        ...
        axios.post(url_request, requestBody).then((response) => {
            ...
            console.log(builtSrcUrl)
            var iframeUrlState = {}
            iframeUrlState[IFRAME_URL_IDENTIFIER] = builtSrcUrl
            console.log("attempt to set state")
            AMP.setState(iframeUrlState)
            revealStyling();
        }).catch((error) => {
